 ...
 * @since 8-apr-2003
 */
public class TableInputPlus extends BaseStep implements StepInterface {
  private static Class<?> PKG = TableInputPlusMeta.class; // for i18n purposes, needed by Translator2!!

  private TableInputPlusMeta meta;
  private TableInputPlusData data;

  public TableInputPlus( StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, TransMeta transMeta,
    Trans trans ) {
  ...
  public void stopRunning( StepMetaInterface smi, StepDataInterface sdi ) throws KettleException {
    meta = (TableInputPlusMeta) smi;
    data = (TableInputPlusData) sdi;
  ...
  public boolean init( StepMetaInterface smi, StepDataInterface sdi ) {
    ...
    if ( super.init( smi, sdi ) ) {
      ...
      if ( Const.isEmpty( meta.getSQL() ) ) {
        logError( BaseMessages.getString( PKG, "TableInputPlus.Exception.SQLIsNeeded" ) );
        passed = false;
      }

      if ( meta.getDatabaseMeta() == null ) {
        logError( BaseMessages.getString( PKG, "TableInputPlus.Exception.DatabaseConnectionsIsNeeded" ) );
        passed = false;
      }
      if ( !passed ) {
        return false;
      ...
      if ( meta.getDatabaseMeta() == null ) {
        logError( BaseMessages.getString( PKG, "TableInputPlus.Init.ConnectionMissing", getStepname() ) );
        return false;
      }
